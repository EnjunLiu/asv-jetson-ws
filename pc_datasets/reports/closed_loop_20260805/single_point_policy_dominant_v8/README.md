...
fail-closed = apply count`. Per-scene metrics are in `red4m/metrics.json` and
`blue3m/metrics.json`; `combined_metrics.json` is their committed combined
copy. All committed log references are relative to this report directory.
