...
if (-not $PythonExe) {
    $PythonExe = Join-Path $DataRoot ".venv-day13\Scripts\python.exe"
}
...
foreach ($Path in $RequiredPaths) {
    if (-not (Test-Path $Path)) {
        throw "Required Day13 path does not exist: $Path"
    }
}
...
Invoke-Day13Python -Name "day13_pc_build" -Arguments @(
...
Invoke-Day13Python -Name "day13_pc_compare" -Arguments @(
