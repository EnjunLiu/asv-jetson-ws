...
$log = Join-Path $env:TEMP "day19-seed${SceneSeed}-$stamp.ue.log"
$ueArguments = @(
    ...
    "-Day12Auto",
    "-Day12Slot=$SlotId",
    "-Day12Layout=$LayoutId",
    "-Day12Motion=$MotionState",
    "-Day12Seed=$SceneSeed",
    "-Day12MaxRuntimeSeconds=$RunSeconds",
    "-RenderOffscreen",
    "-unattended",
    "-nosplash",
    "-stdout",
...
if (-not $ueProcess.HasExited) {
    ...
    Write-Host "DAY19_UE_TIMEOUT_KILLED"
}
$ueProcess.WaitForExit()

Write-Host "DAY19_UE_LOG path=$log"
Write-Host "DAY19_UE_EXIT code=$($ueProcess.ExitCode)"
