...
Copy-Item -Force `
    (Join-Path $automationSource "Day12AutomationSubsystem.h") `
    (Join-Path $projectSource "Day12AutomationSubsystem.h")
Copy-Item -Force `
    (Join-Path $automationSource "Day12AutomationSubsystem.cpp") `
    (Join-Path $projectSource "Day12AutomationSubsystem.cpp")

Write-Host "DAY12_UE_INSTALL_PASS source=$projectSource"
...
Write-Host "DAY12_UE_BUILD_PASS target=EDGEEditor"
