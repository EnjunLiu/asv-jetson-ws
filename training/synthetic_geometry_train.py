...
try:
    ...
    if exc.name != "asv_vla":
        raise
    _source_root = Path(__file__).resolve().parents[1] / "src" / "asv_vla"
    if str(_source_root) not in sys.path:
        sys.path.insert(0, str(_source_root))
    from asv_vla.expert_trajectory import (  # noqa: E402
        ExpertActionResult,
        generate_expert_trajectory,
        task_from_labels,
    )
    from asv_vla.task_entity_tensor import build_entity_tensor  # noqa: E402
...
def load_model_config(path: str | Path) -> Any:
    ...
    from training.model import SmallPolicyConfig
...
def train_synthetic_policy(
    ...
    *,
    ...
    torch = _load_torch()
    from training.model import SmallActionPolicy
...
def _parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--model-config",
        type=Path,
        default=Path("training/config/model_small_v3.yaml"),
    )
    parser.add_argument(
        "--output",
        type=Path,
