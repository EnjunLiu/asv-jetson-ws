...
from training.contract_checks import run_contract  # noqa: E402
...
def _write_cache(path: Path, *, color_leak: bool = False) -> None:
    ...
    np.savez_compressed(
        ...
        frame_keys=np.asarray(
            [
                f"RUN_03:140001:{index}:{(index + 1) * 100000}"
    ...
    (path / "manifest.json").write_text(
        json.dumps({"run_id": "RUN_03"}), encoding="utf-8"
    )
...
def test_dataset_filters_runs_frames_and_language_without_privilege(
    ...
    dataset = FrozenFeatureDataset(
        ...
        split_assignments={"RUN_03": "train"},
