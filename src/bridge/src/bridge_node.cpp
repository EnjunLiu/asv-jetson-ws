...
#include <interfaces/msg/camera_frame.hpp>
#include <interfaces/msg/asv_state.hpp>
#include <interfaces/msg/entity.hpp>
#include <interfaces/msg/entity_array.hpp>
#include <interfaces/msg/desired_displacement.hpp>
...
public:
  ...
  : Node("bridge_node")
  {
    ...
    asv_state_pub_ = create_publisher<interfaces::msg::ASVState>(
      "/ue/asv_state", rclcpp::QoS(10).reliable());

    camera_pub_ = create_publisher<interfaces::msg::CameraFrame>(
      "/ue/camera_frame", rclcpp::QoS(2).best_effort());

    entities_pub_ = create_publisher<interfaces::msg::EntityArray>(
    ...
    if (outbound_command_mode_ == "kinematic") {
      kinematic_setpoint_sub_ =
        create_subscription<interfaces::msg::DesiredDisplacement>(
          "/control/desired_displacement",
          rclcpp::QoS(10).reliable(),
          [this](
            const interfaces::msg::DesiredDisplacement::SharedPtr message)
          {
            RCLCPP_INFO(
              get_logger(),
...
private:
  ...
  void process_json(const std::string & text)
  {
    try {
      ...
      bool valid =
        read_value(body, {"Time", "time"}, simulation_time) &&
        read_value(
          body,
      ...
      if (valid) {
        valid =
          read_value(*asv_location, {"X", "x"}, asv_x) &&
          read_value(*asv_location, {"Y", "y"}, asv_y) &&
          read_value(*asv_location, {"Z", "z"}, asv_z) &&
          read_value(*asv_rotation, {"Roll", "roll"}, roll_degrees) &&
          read_value(*asv_rotation, {"Pitch", "pitch"}, pitch_degrees) &&
          read_value(*asv_rotation, {"Yaw", "yaw"}, yaw_degrees);
      }
  ...
  bool validate_frame_metadata(
    ...
    std::string & detail)
  {
    if (!read_value(body, {"Run_ID", "Run_Id", "RunId", "run_id"}, metadata.run_id)) {
  ...
  {
    interfaces::msg::EntityArray output;
    ...
    const std::string & metadata_detail)
  {
    ...
    for (std::size_t index = 0; index < entities->size(); ++index) {
      ...
      interfaces::msg::Entity entity;
      bool is_target = false;
      bool visible = false;
      const bool metadata_valid =
        read_value(source, {"Entity_Id", "EntityId", "entity_id"}, entity.entity_id) &&
        read_value(source, {"Class", "class"}, entity.class_name) &&
        read_value(source, {"Color", "color"}, entity.color) &&
        read_value(source, {"Is_Target", "IsTarget", "is_target"}, is_target) &&
        read_value(source, {"Visible", "visible"}, visible);
      ...
      const bool vectors_valid =
        ...
        read_value(*position, {"X", "x"}, relative_x) &&
        read_value(*position, {"Y", "y"}, relative_y) &&
        read_value(*position, {"Z", "z"}, relative_z) &&
        read_value(*velocity, {"X", "x"}, velocity_x) &&
        read_value(*velocity, {"Y", "y"}, velocity_y) &&
        read_value(*velocity, {"Z", "z"}, velocity_z);
    ...
    const FrameMetadata & metadata)
  {
    ...
    interfaces::msg::CameraFrame frame;
  ...
  void send_kinematic_setpoint(
    const interfaces::msg::DesiredDisplacement & command)
  {
    const bool finite_step =
      std::isfinite(command.step_dt) && command.step_dt > 0.0F &&
      std::isfinite(command.desired_x) &&
      std::isfinite(command.desired_y);
    const bool executable =
      command.valid && !command.safe_stop && finite_step &&
      !command.run_id.empty() && command.scene_seed > 0 &&
      command.frame_id == "base_link";
    const bool hold_position = !executable;
    const double delta_x_cm = hold_position ? 0.0 :
      static_cast<double>(command.desired_x) * kinematic_position_scale_;
    const double delta_y_cm = hold_position ? 0.0 :
      static_cast<double>(command.desired_y) *
    ...
    const json response = {
      ...
      {"Source_Stamp_Us", command.stamp_us},
      {"Run_ID", command.run_id},
      {"Scene_Seed", command.scene_seed},
      {"Source_Frame_Index", command.frame_index},
      {"Sequence", command.frame_index},
      {"Frame_ID", "ue_actor_local"},
      {"Source_Model_Version", command.source},
  ...
  rclcpp::Publisher<interfaces::msg::ASVState>::SharedPtr asv_state_pub_;
  rclcpp::Publisher<interfaces::msg::CameraFrame>::SharedPtr camera_pub_;
  rclcpp::Publisher<interfaces::msg::EntityArray>::SharedPtr entities_pub_;
  rclcpp::Publisher<std_msgs::msg::Bool>::SharedPtr connected_pub_;
  rclcpp::Publisher<rosgraph_msgs::msg::Clock>::SharedPtr clock_pub_;
  rclcpp::Subscription<interfaces::msg::DesiredDisplacement>::SharedPtr
    kinematic_setpoint_sub_;
};
