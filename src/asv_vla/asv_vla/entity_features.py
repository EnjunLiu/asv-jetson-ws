...
try:
    ...
    from asv_jetson_interfaces.msg import EntityArray, EntityFeatures
except ModuleNotFoundError:  # Keep pure feature tests runnable outside ROS.
    rclpy = None
    Node = object
    HistoryPolicy = QoSProfile = ReliabilityPolicy = None
    EntityArray = EntityFeatures = object
...
class EntityFeaturesNode(Node):
    ...
    def on_entities(self, source: EntityArray) -> None:
        if (
            ...
            self._publish_invalid(source, f"UNTRUSTED_ENTITY_SOURCE:{source.source!r}")
            return
        if not source.valid:
