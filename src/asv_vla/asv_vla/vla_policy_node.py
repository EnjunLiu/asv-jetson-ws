"""CUDA decision-head node for one-step ASV displacement commands.

The perception chain owns image understanding and temporal velocity:

    camera -> image perception -> temporal tracker -> TaskFeatures

This node consumes only ``TaskEmbedding`` and the structured ``TaskFeatures``
...
from asv_jetson_interfaces.msg import DecisionPoint, TaskEmbedding, TaskFeatures
...
POLICY_MODEL_VERSION = "vla_torch_cuda_action_history_v1"
ENTITY_COUNT = 16
LANGUAGE_DIM = 256
...
def task_features_identity_reason(
    message: Any,
...
class VLAPolicyNode(Node):
    ...
    def __init__(self, model_path: str = "") -> None:
        ...
        self._gate_sub = self.create_subscription(
            DecisionPoint, "/vla/selected_point", self._on_gate_result, 10
        )
        self._pub = self.create_publisher(DecisionPoint, "/vla/policy_point", 10)
    ...
    def _on_gate_result(self, message: DecisionPoint) -> None:
    ...
    def _on_language(self, message: TaskEmbedding) -> None:
        ...
        if self._language_task_key != task_key:
            ...
            # history.  The next matching TaskFeatures frame starts cold.
    ...
    def _on_entities(self, message: TaskFeatures) -> None:
        self._entities = message
        now = time.monotonic()
        identity = _identity_tuple(message)
        continuity_reason = task_features_identity_reason(
    ...
    def _new_output(self, ent: TaskFeatures) -> DecisionPoint:
    ...
    def _build_inputs(
        ...
        *,
        ...
        if int(entities.max_entities) != ENTITY_COUNT:
            raise ValueError("TaskFeatures max_entities does not match policy contract")
        if int(entities.feature_dim) != ENTITY_GEOMETRY_DIM:
            raise ValueError("TaskFeatures feature_dim does not match policy contract")
