...
def main(args=None) -> None:
    ...
    # Warmup: send a valid policy first to establish healthy state.
    stamp = int(time.time() * 1e6)
    probe.publish_and_wait(_step_trajectory(0.1, 0.0), stamp_us=stamp)
    time.sleep(0.3)

    # 1. Normal FOLLOW trajectory passes.
    probe.verify("normal_follow", _step_trajectory(0.1, 0.0), expect_pass=True, stamp_us=stamp + 100000)
