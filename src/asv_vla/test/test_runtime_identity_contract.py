...
def test_adapter_requires_identity_and_fails_closed() -> None:
    source = (VLA / "ue_setpoint_adapter.py").read_text(encoding="utf-8")
    assert "def _identity_complete" in source
    assert "scene_seed > 0" in source
    assert "frame_index >= 0" in source
    assert "not bool(msg.safe_stop)" in source
    assert "and identity_complete" in source
...
def test_setup_points_merged_nodes_at_algorithm_modules() -> None:
    setup = (REPOSITORY / "src/asv_vla/setup.py").read_text(encoding="utf-8")
    assert '"entity_features = asv_vla.entity_features:main"' in setup
    assert '"temporal_entity_tracker = asv_vla.temporal_entity_tracker:main"' in setup
    assert '"safety_gate = asv_vla.safety_gate:main"' in setup
