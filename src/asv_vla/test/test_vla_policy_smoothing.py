...
from asv_vla.trajectory_contract import MAX_DISPLACEMENT_M
...
PolicyNode, PendingAction, _task_features_identity_reason = _load_gate_runtime()
...
def test_task_feature_forward_gap_is_valid_but_rollback_is_rejected() -> None:
    first = _point(4, stamp_us=500)
    assert _task_features_identity_reason(first) is None
    assert (
        _task_features_identity_reason(
