...
motion_state=$3
scene_seed=$4
collection_plan=${5:-training/config/sine_near_collection_plan_v1.json}
...
[[ $scene_seed =~ ^[0-9]+$ ]] || {
  echo "SCENE_REMOTE_FAIL invalid scene_seed=$scene_seed" >&2
  exit 2
}
...
automation_root="$repo_root/artifacts/automation"
...
if ss -ltnH 'sport = :8080' | grep -q .; then
  echo "SCENE_REMOTE_FAIL TCP port 8080 already has a listener" >&2
  exit 1
fi

echo "SCENE_REMOTE_START slot=$slot_id layout=$layout_id motion=$motion_state scene_seed=$scene_seed rollout_action=$rollout_action"

setsid env PYTHONUNBUFFERED=1 ros2 launch asv_bringup collect.launch.py \
  slot_id:="$slot_id" \
  layout_id:="$layout_id" \
...
while (( SECONDS < ready_deadline )); do
  if ! kill -0 "$launch_pid" 2>/dev/null; then
    echo "SCENE_REMOTE_FAIL ROS launch exited before readiness" >&2
    wait "$launch_pid" || true
    exit 1
  fi
  if grep -q "EPISODE_RECORDER_READY" "$launch_log" \
    && ss -ltnH 'sport = :8080' | grep -q .; then
    echo "SCENE_REMOTE_READY slot=$slot_id layout=$layout_id motion=$motion_state scene_seed=$scene_seed"
    break
  fi
  sleep 0.2
done
...
if ! grep -q "EPISODE_RECORDER_READY" "$launch_log" \
  || ! ss -ltnH 'sport = :8080' | grep -q .; then
  echo "SCENE_REMOTE_FAIL timed out waiting for recorder and TCP listener" >&2
  exit 1
fi

completion_seen=false
while kill -0 "$launch_pid" 2>/dev/null; do
  if grep -q "EPISODE_RECORDING_COMPLETE episode=" "$launch_log"; then
...
if [[ $completion_seen != true ]]; then
  echo "SCENE_REMOTE_FAIL ROS launch exited before recording completed status=$launch_status" >&2
  exit 1
fi
...
episode_dir=$(
  grep "EPISODE_RECORDING_COMPLETE episode=" "$launch_log" \
    | tail -n 1 \
    | sed -E 's/.*episode=([^ ]+) frames=.*/\1/'
)
if [[ -z $episode_dir || ! -d $episode_dir ]]; then
  echo "SCENE_REMOTE_FAIL no completed episode was found in $launch_log" >&2
  exit 1
fi
...
python3 -m training.collection status \
  --data-root . \
  --plan "$collection_plan" \
  --report artifacts/collection_report_v1.json
python3 -m training.dataset_registry \
  --data-root . \
  --output artifacts/registry/dataset_registry_v1.jsonl
python3 -m training.make_group_splits \
  --registry artifacts/registry/dataset_registry_v1.jsonl \
  --output artifacts/registry/group_split_v1.json \
  --instructions dataset/language/instructions.jsonl
...
echo "SCENE_PACKAGE path=$package_path sha256=$package_sha"
echo "SCENE_REMOTE_COMPLETE slot=$slot_id run_id=$run_id"
